...
export const SUMMARIES: Record<string, string> = {
  ...
  "q-cs-9": "지갑 vs 열쇠",
  "q-cs-10": "이름 ↔ 숫자 해석기",
  "q-cs-11": "개인키=서명, 공개키=검증",
  "q-cs-12": "소인수분해 · 암호화도 가능",
  "q-cs-13": "난수 재사용에 취약 · 퇴역",
  "q-cs-14": "타원곡선 · 짧은 키로 같은 안전도",
  "q-cs-15": "난수를 해시로 결정 · 실수 차단",
  "q-cs-16": "256비트 서명 전용 · 모바일 기본값",
  "q-os-1": "코드·데이터·스택 분리 = 공유로 절약",
  "q-os-2": "초인종 vs 계속 나가서 확인하기",
  "q-os-3": "fork=복제, exec=통째로 교체",
  "q-os-4": "프로세스 신상명세서",
  "q-os-5": "책갈피 꽂고 펴는 순수 오버헤드",
  "q-os-6": "커널이 놔주는 통로 6종",
  "q-os-7": "마이크를 뺏을 수 있나 없나",
  "q-os-8": "4조건 모두 성립해야 발생",
  "q-os-9": "접근 순서에 따라 결과가 바뀜",
  "q-os-10": "열쇠 1개 vs 주차장 N자리",
  "q-os-11": "동시 접근해도 의도대로 동작",
  "q-os-12": "고정=내부단편화, 가변=외부단편화",
  "q-os-13": "가짜 주소를 진짜 주소로 번역",
  "q-os-14": "먼저온놈 vs 오래안쓴놈 vs 미래",
  "q-net-1": "층별로 나눠야 층별로 고친다",
  "q-net-2": "수신자 책상 넘침 방지",
  "q-net-3": "네트워크 도로 혼잡 방지",
  "q-net-4": "종료는 할 말이 남아서 4번",
  "q-net-5": "등기우편 vs 전단지",
  "q-net-6": "서버를 늘려서 나눠 받기",
  "q-net-7": "느린 공개키로 빠른 대칭키를 전달",
  "q-net-8": "인증서 확인 후 대칭키 합의",
  "q-net-9": "캐시 → Root → TLD → 담당 서버",
  "q-net-10": "멈춰 기다리나, 다른 일 하나",
  "q-net-11": "제어권 축 vs 완료확인 축",
  "q-net-12": "연결을 열어둔 양방향 통신",
  "q-db-1": "최소성 있으면 후보키, 없으면 슈퍼키",
  "q-db-2": "삽입·갱신·삭제 시 생기는 부작용",
  "q-db-3": "원자값 → 부분종속 → 이행종속 제거",
  "q-db-4": "책 뒤 찾아보기 — 조회↑ 수정↓",
  "q-db-5": "해시는 범위 검색을 못 한다",
  "q-db-6": "전부 아니면 전무 + 4가지 보장",
  "q-db-7": "격리↑ 안전↑ 속도↓ 4단계",
  "q-db-8": "교집합이냐, 한쪽을 다 살리냐",
  "q-db-9": "입력값이 문법이 되어버리는 공격",
  "q-db-10": "정해진 양식 vs 뭉쳐 담기",
  "q-db-11": "미리 만들어 빌려주고 반납받기",
  "q-db-12": "가로로 자르면 샤딩",
  "q-db-13": "RAM 에 두는 Key-Value 저장소",
  "q-ds-1": "극장 좌석 vs 보물찾기 쪽지",
  "q-ds-2": "접시 쌓기(LIFO) vs 줄 서기(FIFO)",
  "q-ds-3": "최댓값을 빠르게 꺼내는 반정렬 트리",
  "q-ds-4": "사이클 없고 모두 연결되면 트리",
  "q-ds-5": "한쪽으로 자라면 연결 리스트가 됨",
  "q-ds-6": "체이닝으로 매달거나 빈칸 찾아가기",
  "q-ds-7": "접두사를 공유하는 문자열 트리",
  "q-ds-8": "노드를 크게 = 디스크 읽는 횟수↓",
  "q-ds-9": "모든 경로면 DFS, 최단거리면 BFS",
  "q-ds-10": "평균은 퀵, 보장은 병합, 메모리는 힙",
  "q-ds-11": "입력이 커질 때 늘어나는 모양",
  "q-ds-12": "사물함(빠름) vs 사전(정렬됨)",
  ...
  "q-arch-5": "바뀌는 이유가 같은 것끼리",
  "q-arch-6": "껍데기를 통일해 분기 줄이기",
  "q-arch-7": "안 써도 되면 안 쓰는 게 낫다",
  "q-arch-8": "코드를 현실 업무 말로 맞추기",
  "q-arch-9": "서버가 기억을 안 하면 늘리기 쉽다",
  "q-arch-10": "이름 잘 짓기 — 가장 싼 문서",
  "q-interview-1": "정의 → 특징 → 내 경험 3층",
  "q-interview-2": "객체 조립(DI) + 공통 잡일(AOP)",
  "q-interview-3": "홀과 주방을 나눈다",
  "q-interview-4": "전부 성공 아니면 전부 롤백 선언",
  "q-interview-5": "문제·후보·기준·포기한 것",
  "q-interview-6": "읽고 설명하고 검증했는가",
  "q-interview-7": "정의·이유·장단점·사용처",
  "q-interview-8": "내 요구사항 먼저, 그다음 비교",
  "q-interview-9": "에러와 씨름하며 디버깅을 배운다",
  "q-interview-10": "코드를 판단하는 능력이 남는다",
};
