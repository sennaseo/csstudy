...
이론(CS 퀴즈)과 실무(BackendQuest 실험판), 두 갈래로 굴러간다.
...
| 버디 캐릭터 | 12종 × 3단계 진화 |
| 실무 프로토타입 | 챕터 3 첫 레슨 7문제 (BackendQuest) |
...
- **난이도 선택** — 믹스/초급/중급/고급. 쉬운 문제만 볼지 어려운 문제만 볼지, 그리고 얼마나 어렵게 물을지(고르기 → 빈칸 → 직접 쓰기·말하기)를 함께 바꾼다. 문제 데이터·진행도는 그대로.
...
### 하단 탭
화면 하단에 5개 탭이 고정된다: **홈 · 이론 · 실무 · 도감 · 마이**.

| 탭 | 내용 |
| --- | --- |
| 홈 | 목표 게이지 · 트랙·난이도 고르기 · 원탭 시작 · 버디 |
| 이론 | 트랙 진행도 + 감긴 길(스킬 패스) — CS 퀴즈 139문제 |
| 실무 | BackendQuest 프로토타입 |
| 도감 | 캐릭터 도감 |
| 마이 | 연속 학습 · 하트 · 단어장 · 클라우드 동기화 |

#### 실무 탭
- `public/practice/index.html` 을 iframe 으로 연다.
- **진도 저장 없음** — 새로고침하면 날아간다. 아직 검증용 실험판.
- 기획서·문제 스키마는 `practice/` 에 있다 (`백엔드퀘스트_기획서.md`, `schema/`).
- 이론(CS 퀴즈)과는 **별개 엔진**이다 — 데이터 모델·채점 방식이 다르고 하트도 쓰지 않는다.
...
| `practice/selftest.cjs` | 실무 프로토타입 7문제 정·오답 판정 · 하트 잔존 0 · 코드 누적 |
...
│  └─ practice/            # 실무(BackendQuest) 프로토타입 index.html
├─ practice/                # 실무 기획서 · 문제 스키마 · 자체 점검
│  ├─ 백엔드퀘스트_기획서.md
│  ├─ schema/              # problem-schema.md · problems-sample.json · check_apply.py
│  └─ selftest.cjs
